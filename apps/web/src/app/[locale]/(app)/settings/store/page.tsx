...
export default function StoreSettingsPage() {
  ...
  return (
    <main className="mx-auto max-w-xl px-6 py-10">
      ...
      {readOnly && <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{t('ownerOnly')}</p>}
      ...
      <form onSubmit={onSubmit} className="mt-6 flex flex-col gap-4">
        <label className="flex flex-col gap-1 text-sm">
          ...
          <input
            ...
            disabled={readOnly}
            className="rounded-md border border-slate-300 px-3 py-2 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 disabled:bg-slate-50"
          />
        </label>
        <label className="flex flex-col gap-1 text-sm">
          {t('localeLabel')}
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <div className="text-xs text-slate-500 dark:text-slate-400">{t('subscriptionPlan')}</div>
            <div className="mt-1 font-medium">{store.subscriptionPlan}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500 dark:text-slate-400">{t('currency')}</div>
